imgUv = imread('pictures/IMG_0001.JPG');
imgWh = imread('pictures/IMG_0002.JPG');

imshow(imgWh)
% click 4 points clockwise from the top-left cornor
[px, py] = ginput(4);

imgUv = imgUv(min(py): max(py), min(px): max(px), 3); % extract blue component
imgWh = imgWh(min(py): max(py), min(px): max(px), :);
imwrite(imgUv, 'uv.jpg');
imwrite(imgWh, 'Wh.jpg');
imgUvBin = im2bw(imgUv, 0.15);
imshow(imgUvBin)
